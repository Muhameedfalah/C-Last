...
void getLocation() {
    ...
    struct utsname unameData; // משתנה לאחסון מידע על המחשב
    if (uname(&unameData) != 0) { // בדיקה אם הצליח לקבל מידע על המחשב
        puts("Error getting system information");
        return;
    }

    printf("\033[1;32m"); // הדגשה בצבע ירוק
    printf("%s@", unameData.nodename); // מדפיס את שם המשתמש
    printf("\033[1;34m"); // הדגשה בצבע כחול
    printf("%s:", unameData.machine); // מדפיס את שם המחשב
    printf("\033[0;36m"); // צבע טקסט תכלת
    printf("%s$ ", location); // מדפיס את הנתיב הנוכחי
    printf("\033[0m"); // איפוס הצבעים לברירת מחדל
}
...
#include <stdio.h>
#include <unistd.h>
#include <sys/utsname.h> // כולל את struct utsname שמכיל מידע על המחשב
...
int main() {
    getLocation(); // קריאה לפונקציה להדפסת המיקום הנוכחי
    return 0;
}*/
